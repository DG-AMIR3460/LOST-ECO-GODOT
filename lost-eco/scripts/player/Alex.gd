...
func _animate_idle(delta: float) -> void:
	_walk_t = 0.0
	var s := delta * 14.0
	if leg_l: leg_l.position.y = lerpf(leg_l.position.y, 0.0, s)
	if leg_r: leg_r.position.y = lerpf(leg_r.position.y, 0.0, s)
	if arm_l: arm_l.position.y = lerpf(arm_l.position.y, 0.0, s)
	if arm_r: arm_r.position.y = lerpf(arm_r.position.y, 0.0, s)
