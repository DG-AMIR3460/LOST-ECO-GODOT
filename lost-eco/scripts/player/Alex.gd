...
# ── Visual del personaje (construido con polígonos) ───────────────────────────
...
func _build_character() -> void:
	...
	# Cabello (encima de la cara)
	_r(visual, -4, -9,  8, 3, HAIR)
	# Patillas / laterales del cabello
	...
	# ── Brazos (guardamos referencia para animación) ─────────────────────────
	arm_l = _r(visual, -6, -2,  2, 5, SHIRT)
	arm_r = _r(visual,  4, -2,  2, 5, SHIRT)
	# Manos
	_r(visual, -6,  3,  2, 2, SKIN).name = "hand_l"
	_r(visual,  4,  3,  2, 2, SKIN).name = "hand_r"

	# ── Piernas / pantalón (referencia para animación) ───────────────────────
...
func _animate_walk(dir: Vector2, delta: float) -> void:
	...
	# Flip horizontal al moverse izquierda / derecha
	if dir.x < -0.1:
		visual.scale.x = -1.0
	elif dir.x > 0.1:
		visual.scale.x = 1.0

	# Balanceo sinusoidal de piernas y brazos
	var swing = sin(_walk_t) * 2.8
	if leg_l: leg_l.position.y =  swing
	if leg_r: leg_r.position.y = -swing
	if arm_l: arm_l.position.y = -swing * 0.7
	if arm_r: arm_r.position.y =  swing * 0.7

func _animate_idle(delta: float) -> void:
	_walk_t = 0.0
	# Vuelve suavemente a posición neutral
	var s = delta * 14.0
	if leg_l: leg_l.position.y = lerp(leg_l.position.y, 0.0, s)
	if leg_r: leg_r.position.y = lerp(leg_r.position.y, 0.0, s)
	if arm_l: arm_l.position.y = lerp(arm_l.position.y, 0.0, s)
	if arm_r: arm_r.position.y = lerp(arm_r.position.y, 0.0, s)
...
func _play_attack_animation() -> void:
	# Brazo derecho golpea hacia adelante
	if arm_r and visual:
