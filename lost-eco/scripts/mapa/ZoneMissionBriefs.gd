	...
	match zone_id:
		...
		3:
			DialogueManager.show_zone_intro(
				...
				"META: 3 cristales + 3 luces + 3 ecos + sanar al jefe.\n\n"
				+ "1) Cristales AZULES → [E] (lee la memoria).\n"
				+ "2) Círculos DORADOS → [E] (enciende la luz).\n"
				+ "3) Recoge 3 ecos (★).\n"
				+ "4) Con todo listo: Sala del Espejo → [E] o [Q] para sanar al jefe.",
				"[J] pulso empuja  ·  [Q] gesto de paz  ·  HUD muestra tu progreso",
				Color(0.92, 0.82, 0.40)
			)
		4:
			DialogueManager.show_zone_intro(
				"ZONA 4 — El Río",
				"META: recoger 3 Ecos de Luz.\n\n"
...
static func hud_objective(zone_id: int) -> String:
	match zone_id:
		1: return "META: 4 ECOS → SALIDA X"
		2: return "META: 3 PILARES + 3 ECOS"
		3: return "META: CRIST+LUCES+ECOS+JEFE"
		4: return "META: 3 ECOS → SALIDA X"
		_: return ""
